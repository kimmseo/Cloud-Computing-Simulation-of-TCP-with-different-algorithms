...
def simulate_tcp_aimd():
    ...
    while bytes_sent < DATA_SIZE:
        bytes_to_send = min(cwnd * MSS, DATA_SIZE - bytes_sent)
        ...
        if random.random() < 0.01:
            cwnd = max(1, int(cwnd * AIMD_DECREMENT))
        else:
            cwnd += AIMD_INCREMENT
        ...
        if elapsed_time - (last_time - start_time) >= SAMPLE_INTERVAL:
            ...
            print(f"Time: {elapsed_time:.2f}s, Throughput: {throughput:.2f} MB/s, cwnd: {cwnd} segments")
...
def main():
    ...
    print("Simulating TCP AIMD throughput...")

    times, throughputs = simulate_tcp_aimd()

    if times and throughputs:
        plt.plot(times, throughputs, label='Throughput (MB/s)', color='blue')
        plt.xlabel('Time (seconds)')
        plt.ylabel('Throughput (MB/s)')
        plt.title('TCP AIMD Throughput Simulation')
        plt.legend()
        plt.grid(True)
        plt.show()
    else:
